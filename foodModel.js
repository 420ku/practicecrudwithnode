...
const foodModel = {
  ...
  getFoodsById: function (recipeId) {
    return sql`SELECT * FROM recipes WHERE recipe_id = ${recipeId};`;
  },

  getFoodsByType: function (type) {
    return sql`SELECT
...
FROM
    recipes r
JOIN
    ingredients i ON r.recipe_id = i.recipe_id WHERE r.type = ${type};
    `;
  },

  addRecipe: function ({ name, type, instructions }) {
    return sql`INSERT INTO recipes (name,type,instructions) VALUES (${name}, ${type}, ${instructions}) RETURNING recipe_id,name,type,instructions;`;
  },
  ...
  addIngredientsToRecipe: function (recipeId, name, quantity, condition) {
    return sql`
      ...
      VALUES (${recipeId}, ${name}, ${quantity}, ${condition}) RETURNING *;
    `;
  },
  ...
  editIngredientsByRecipe: function (recipeId, name, quantity, condition) {
    ...
    quantity = ${quantity},
    condition = ${condition}
    WHERE recipe_id = ${recipeId} RETURNING *;
      `;
  },
...
WHERE recipes.recipe_id = ingredients.recipe_id
  AND recipes.recipe_id = ${recipeId} RETURNING *;
`;
  },
};
